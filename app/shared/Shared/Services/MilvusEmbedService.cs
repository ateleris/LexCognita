...
public sealed partial class MilvusEmbedService(
...
,
    ...
    private async Task IndexSectionsAsync(IEnumerable<Section> sections)
    {
        ...
        foreach (var section in sections)
        {
            ...
            categories.Add(section.Category ?? string.Empty);
            sourcepages.Add(section.SourcePage);
            sourcefiles.Add(section.SourceFile);
            ...
            if (iteration % batch == 0)
            {
                ...
                await collection.InsertAsync([
                    FieldData.Create<string>("id", ids),
                    FieldData.Create<string>("content", contents),
                    FieldData.Create<string>("category", categories),
                    FieldData.Create<string>("sourcepage", sourcepages),
                    FieldData.Create<string>("sourcefile", sourcefiles),
                    FieldData.CreateFloatVector("embedding", embeds),
                ]);
